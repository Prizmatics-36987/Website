(function () {
    ...
    function engineLoop() {
        ...
        var ease = mqReduce.matches ? 1 : 0.085;
    ...
    function updateDeck(a) {
        ...
        frontIdx = idx;
        if (c3dStatus) {
            c3dStatus.innerHTML = "STAGE " + pad(idx + 1, 2) + " <b>//</b> " + modNames[idx];
        }
    ...
    var tabData = {
        performance: {
            label: "MEDIUM // EMAIL",
            value: "1 WEEK",
            href:  "mailto:prismatics@prizma.org.il",
        },
        range: {
    ...
    function renderTab(key) {
        var d = tabData[key];
        const href = kValue.getAttribute("href");
        if (!d) {
            return;
        }
        kLabel.textContent = d.label;
        kValue.setAttribute("href", d.href);
        if (mqReduce.matches) {
